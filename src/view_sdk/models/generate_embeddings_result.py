...
from ..models.content_embedding import (
    ContentEmbeddingModel,
)  # Placeholder for ContentEmbedding
...
class GenerateEmbeddingsResultModel(BaseModel):
    ...
    semantic_cells: List[SemanticCellModel] = Field(
        default_factory=list, alias="SemanticCells"
    )
    content_embeddings: List[ContentEmbeddingModel] = Field(
        default_factory=list, alias="ContentEmbeddings"
    )
