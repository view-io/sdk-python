...
def test_exists_credential_success(config_api, guid):
    ...
    with patch("viewio_sdk.api_client.ApiClient.call_api") as mock_credential:
        ...
        exists = config_api.exists_credential_without_preload_content(guid, guid)
...
def test_exists_credential_not_found(config_api, guid, error_code, expected_status):
    # Mock the ApiException
    mock_response = _get_failure_mock_response(expected_status, error_code)

    with patch("viewio_sdk.api_client.ApiClient.call_api") as mock_create_credential:
        mock_create_credential.side_effect = ApiException(http_resp=mock_response)

        with pytest.raises(ApiException) as exc_info:
            config_api.exists_credential_without_preload_content(guid, guid)
...
def test_create_credential_success(config_api, guid):
    ...
    with patch("viewio_sdk.api_client.ApiClient.call_api") as mock_create:
        ...
        credential = config_api.create_credential_without_preload_content(guid, new_credential)

        assert credential.data == new_credential.to_json().encode("utf-8")
...
def test_create_credential_api_errors(config_api, guid, error_code, expected_status):
    ...
    with patch("viewio_sdk.api_client.ApiClient.call_api") as mock_create:
        ...
        with pytest.raises(ApiException) as exc_info:
            config_api.create_credential_without_preload_content(guid, Credential(**credential_data))
...
# Test for successfully deleting a credential
def test_delete_credential_success(config_api, guid):
    new_credential = Credential(
        guid=str(uuid4()),
        tenant_guid=str(uuid4()),
        user_guid=str(uuid4()),
    ...
    with patch("viewio_sdk.api_client.ApiClient.call_api") as mock_delete:
        ...
        credential = config_api.delete_credential_without_preload_content(guid, credential_guid)

        assert credential.data == new_credential.to_json().encode("utf-8")


# Parametrized test for various API error scenarios for deleting a credential
@pytest.mark.parametrize(
    "error_code, expected_status",
    [
        (ApiErrorEnum.BADREQUEST, 400),
...
def test_delete_credential_api_errors(config_api, guid, error_code, expected_status):
    credential_guid = str(uuid4())
    # Mock the ApiException
    mock_response = _get_failure_mock_response(expected_status, error_code)
    with patch("viewio_sdk.api_client.ApiClient.call_api") as mock_delete:
        mock_delete.side_effect = ApiException(http_resp=mock_response)

        with pytest.raises(ApiException) as exc_info:
            config_api.delete_credential_without_preload_content(guid, credential_guid)
...
def test_retrieve_credentials_success(config_api, guid):
    ...
    with patch("viewio_sdk.api_client.ApiClient.call_api") as mock_get:
        ...
        retrieved_credentials = config_api.retrieve_credentials_without_preload_content(guid)

        assert retrieved_credentials.data == json.dumps(credentials_json).encode("utf-8")
...
def test_retrieve_credentials_api_errors(config_api, guid, error_code, expected_status):
    ...
    with patch("viewio_sdk.api_client.ApiClient.call_api") as mock_retrieve:
        ...
        with pytest.raises(ApiException) as exc_info:
            config_api.retrieve_credentials_without_preload_content(guid)
...
def test_retrieve_credential_success(config_api, guid):
    ...
    with patch("viewio_sdk.api_client.ApiClient.call_api") as mock_get:
        mock_get.return_value = mock_response
        credential = config_api.retrieve_credential_without_preload_content(guid, credential_guid)

        assert credential.data == existing_credential.to_json().encode("utf-8")
...
def test_retrieve_credential_api_errors(config_api, guid, error_code, expected_status):
    ...
    with patch("viewio_sdk.api_client.ApiClient.call_api") as mock_get:
        ...
        with pytest.raises(ApiException) as exc_info:
            config_api.retrieve_credential_without_preload_content(guid, credential_guid)
...
def test_update_credential_success(config_api, guid):
    ...
    with patch("viewio_sdk.api_client.ApiClient.call_api") as mock_update:
        ...
        credential = config_api.update_credential_without_preload_content(
            guid, credential_guid, updated_credential
        )

        assert credential.data == updated_credential.to_json().encode("utf-8")
...
def test_update_credential_api_errors(config_api, guid, error_code, expected_status):
    ...
    with patch("viewio_sdk.api_client.ApiClient.call_api") as mock_update_credential:
        ...
        with pytest.raises(ApiException) as exc_info:
            config_api.update_credential_without_preload_content(guid, credential.guid, credential)
