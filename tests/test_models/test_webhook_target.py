...
def test_invalid_urls(url):
    ...
    assert (
        "Input should be a valid URL" in error_msg
        or "URL input should be a string" in error_msg
    )
...
def test_invalid_content_types(content_type):
    ...
    assert (
        "Content type must not be empty" in error_msg
        or "Input should be a valid string" in error_msg
    )


@pytest.mark.parametrize(
    "expect_status,expected_error",
    [
...
def test_invalid_expect_status(expect_status, expected_error):
    ...
    with pytest.raises(ValidationError) as exc_info:
        WebhookTargetModel(Url="http://example.com/webhook", ExpectStatus=expect_status)
...
def test_valid_content_types(content_type):
    ...
    target = WebhookTargetModel(
        Url="http://example.com/webhook", ContentType=content_type
    )
