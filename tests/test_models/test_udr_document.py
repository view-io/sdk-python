...
class TestUdrDocumentModel:
    ...
    def test_error_handling(self, error_response):
        doc = UdrDocumentModel(Type=DocumentTypeEnum.Unknown, Error=error_response)
    ...
    def test_top_terms_with_empty_strings(self):
        doc = UdrDocumentModel(
            Type=DocumentTypeEnum.Unknown, Terms=["term1", "", "term1", "", "term2"]
        )
    ...
    def test_semantic_cells(self):
        chunk = SemanticChunkModel(Content="test cell content", Position=0)
        cell = SemanticCellModel(
            CellType=SemanticCellTypeEnum.Text, Position=0, Chunks=[chunk]
        )
        doc = UdrDocumentModel(Type=DocumentTypeEnum.Unknown, SemanticCells=[cell])
        assert len(doc.semantic_cells) == 1
        assert doc.semantic_cells[0].chunks[0].content == "test cell content"
        assert doc.semantic_cells[0].chunks[0].length == len(
            "test cell content"
        )  # Length is auto-calculated
        assert doc.semantic_cells[0].length == len(
            "test cell content"
        )  # Cell length should match chunk length
