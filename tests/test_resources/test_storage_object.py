...
def test_object_set_expiration(mock_super):
    ...
    mock_super().update.assert_called_with(
        "obj1", bucket_guid="bucket1", expiration=dt, data={"ExpirationUtc": dt}
    )
...
def test_object_retrieve_range(mock_super):
    ...
    assert result == "range"
    mock_super().retrieve.assert_called_with(
        "obj1", bucket_guid="bucket1", headers={"Range": "bytes=0-99"}
    )
