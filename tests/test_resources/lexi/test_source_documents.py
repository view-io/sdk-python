...
from view_sdk.resources.lexi.healthcheck import HealthCheck
...
class TestSourceDocument:
    ...
    def test_create(self, mock_create, mock_source_document, collection_guid):
        ...
        result = SourceDocument.create(collection_guid=collection_guid, **mock_source_document)

        assert isinstance(result, SourceDocumentModel)
        mock_create.assert_called_once_with(collection_guid=collection_guid, **mock_source_document)

    @patch('view_sdk.resources.lexi.source_documents.RetrievableAPIResource.retrieve')
    def test_retrieve(self, mock_retrieve, mock_source_document, collection_guid, document_guid):
        ...
        assert isinstance(result, SourceDocumentModel)
        mock_retrieve.assert_called_once_with(document_guid, collection_guid=collection_guid)

    @patch('view_sdk.resources.lexi.source_documents.RetrievableAPIResource.retrieve')
    def test_retrieve_with_data(self, mock_retrieve, mock_source_document, collection_guid, document_guid):
        mock_retrieve.return_value = SourceDocumentModel(**mock_source_document)

        result = SourceDocument.retrieve(document_guid, collection_guid, include_data=True)

        assert isinstance(result, SourceDocumentModel)
        mock_retrieve.assert_called_once_with(document_guid, collection_guid=collection_guid, incldata=None)
    ...
    def test_delete(self, mock_delete, collection_guid, document_guid):
        ...
        mock_delete.assert_called_once_with(document_guid, collection_guid=collection_guid)

    @patch('view_sdk.resources.lexi.source_documents.DeletableAPIResource.delete')
    def test_delete_by_key_and_version(self, mock_delete, collection_guid, document_guid):
        key = "test_key"
        version = "1"

        SourceDocument.delete_by_key_and_version(document_guid, collection_guid, key, version)

        mock_delete.assert_called_once_with(document_guid, collection_guid=collection_guid, key=key, versionId=version)
    ...
    def test_exists(self, mock_exists, collection_guid, document_guid):
        ...
        mock_exists.assert_called_once_with(document_guid, collection_guid=collection_guid)

    @patch('view_sdk.resources.lexi.source_documents.RetrievableStatisticsMixin.retrieve_statistics')
    def test_retrieve_statistics(self, mock_stats, collection_guid, document_guid, mock_source_document):
        ...
        mock_source_document.update({
            "UdrDocument": {
                ...
                "Schema_": {
                    "Flattened": [{"key": "value1"}, {"key": "value2"}]
                },
                "SemanticCells": [
                    {
                        "GUID": str(uuid.uuid4()),
        ...
        result = SourceDocument.retrieve_statistics(document_guid, collection_guid=collection_guid)
        ...
        mock_stats.assert_called_once_with(document_guid, collection_guid=collection_guid)

    @patch('view_sdk.resources.lexi.source_documents.EnumerableAPIResourceWithData.enumerate_with_query')
    ...
    @patch('view_sdk.resources.lexi.source_documents.AllRetrievableAPIResource.retrieve_all')
    def test_retrieve_all(self, mock_retrieve_all, mock_source_document, collection_guid):
