...
class TestTypeDetector:
    def test_process_empty_data(self):
        ...
        with pytest.raises(
            ValueError, match="No data supplied for content type detection."
        ):
            TypeDetector.type_detection(data=b"")
    ...
    def test_process_with_data_and_content_type(self, mock_create):
        ...
        expected_result = TypeResultModel(
            mime_type="text/csv", extension=".csv", type_=DocumentTypeEnum.Csv
        )
        mock_create.return_value = expected_result

        # Test the method
        result = TypeDetector.type_detection(
    ...
    def test_process_without_content_type(self, mock_create):
        ...
        # Test the method
        result = TypeDetector.type_detection(data=b"some binary data")

        # Verify interactions
        mock_create.assert_called_once_with(
            data=b"some binary data", content_type="application/octet-stream"
        )
        assert result == expected_result
    ...
    def test_process_create_exception_handling(self, mock_create):
        ...
        # Test the method
        result = TypeDetector.type_detection(data=b"some data")
    ...
    def test_type_result_model_custom_values(self):
        ...
        result = TypeResultModel(
            mime_type="application/pdf", extension=".pdf", type_=DocumentTypeEnum.Pdf
        )
