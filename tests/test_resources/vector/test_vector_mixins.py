...
def test_generate_embeddings_error_handling(mock_client, mock_request):
    ...
    error_model = Mock()
    error_model.return_value = Mock(success=False, status_code=500, model=TestEmbeddingsGenerator.DEFAULT_MODEL)
    ...
    error_model.assert_called_once_with(success=False, status_code=500, model=TestEmbeddingsGenerator.DEFAULT_MODEL)
...
def test_validate_connectivity_success(mock_client):
    ...
    mock_response = Mock()
    mock_client.return_value.request.return_value = mock_response  # Mock the request method
